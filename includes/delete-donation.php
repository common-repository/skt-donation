...
<div>
<?php
$retrieved_nonce = sanitize_text_field($_REQUEST['checknounce']);
if (!wp_verify_nonce($retrieved_nonce, 'my-nonce' ) ){
	die( 'Failed security check' );
} else{
	$delete_id = sanitize_text_field(isset($_GET['delete_id']) ? $_GET['delete_id'] : '');
	$tablename_donation_amount = sanitize_text_field(isset($_GET['tablename']) ? $_GET['tablename'] : '');
	$mode_delete = sanitize_text_field(isset($_GET['mode_delete']) ? $_GET['mode_delete'] : '');
