...
if ( !isset( $_POST['name_of_nonce_field'] ) || !wp_verify_nonce($_POST['name_of_nonce_field'], 'name_of_my_action' ) ){	
	...
	$mode_currency = sanitize_text_field(isset($_POST['mode_currency']) ? $_POST['mode_currency'] : '');
	if($mode_currency !=''){
	...
	switch ($mode_currency) {
		case 'currency_sign_paypal':
			...
			$paypal_sktcurrency_id = sanitize_text_field($_POST['paypal_sktcurrency_id']);
		...
		case 'currency_sign_twocheckout':
			...
			$twocheckout_sktcurrency_id = sanitize_text_field($_POST['twocheckout_sktcurrency_id']);
