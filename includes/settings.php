...
if ( ! function_exists ( 'skt_donation_settings_page' ) ) {
  ...
  ?>
    ...
    <?php
        global $wpdb;
        $msg_success = sanitize_text_field(isset($_GET['msg_success']) ? $_GET['msg_success'] : '');
        if($msg_success !=''){
        $skt_donation_active_tab = sanitize_text_field(isset($_GET['skt_donation_active_tab']) ? $_GET['skt_donation_active_tab'] : '');
        ?>
            <p class="skt_donation_msg_success"><?php echo esc_attr($msg_success);?></p>
        <?php } 
        $msg_failed = sanitize_text_field(isset($_GET['msg_failed']) ? $_GET['msg_failed'] : '');
        if($msg_failed !=''){
        echo $skt_donation_active_tab = sanitize_text_field(isset($_GET['skt_donation_active_tab']) ? $_GET['skt_donation_active_tab'] : '');
    ?>
